...
def process_files(source_folder, dest_base_folder, questions_excel, titles_excel):
    ...
    for i in range(2):  # As we need at least two folders
        os.makedirs(f"{dest_base_folder}/Folder_{i + 1}", exist_ok=True)

    # Step 2: Get the naturally sorted list of .docx files
    ...
    # Step 3: Move 60 .docx files to each of the new folders
    for i in range(2):
    ...
    # Step 4: Read the A columns from the Excel files
    ...
    for i in range(2):
        ...
        # Create txt file for questions
        questions = df_questions.iloc[i * 60:(i + 1) * 60].squeeze().tolist()
        with open(os.path.join(dest_folder, 'questions.txt'), 'w') as f:
            for question in questions:
                f.write(str(question) + '\n')

        # Create txt file for titles
        titles = df_titles.iloc[i * 60:(i + 1) * 60].squeeze().tolist()
        with open(os.path.join(dest_folder, 'titles.txt'), 'w') as f:
            for title in titles:
                f.write(str(title) + '\n')


# Example usage:
source_folder = r'C:\Users\14534\Documents\num_file'
